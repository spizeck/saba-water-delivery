...
async function seed() {
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    await Promise.all([
      ...
      setDoc(doc(db, "waterRequests/request-a/photos/photo-a"), {
        path: "request-photos/request-a/photo-a",
      }),
      setDoc(doc(db, "users/resident-a/propertyPhotos/photo-a"), {
        path: "property-photos/resident-a/photo-a",
      }),
      ...
      setDoc(doc(db, "driverRegistry/registry-a/events/event-a"), {
        type: "driver_online",
      }),
      setDoc(doc(db, "driverRegistry/registry-a/meters/station-a"), {
        meterCode: "42",
      }),
      setDoc(doc(db, "driverRegistryUniqueKeys/name-key"), {
        driverId: "registry-a",
      }),
      setDoc(doc(db, "fillStations/station-a"), {
        name: "Bottom Fill Station",
        active: true,
      }),
      setDoc(doc(db, "driverOffers/offer-a"), {
        driverId: "driver-a",
        requestId: "request-a",
      }),
      setDoc(doc(db, "config/dispatch"), { offerTimeoutSeconds: 60 }),
      setDoc(doc(db, "config/dispatch/events/event-a"), {
        type: "settings_updated",
      }),
      setDoc(doc(db, "dispatchBatches/batch-a"), {
        driverId: "driver-a",
        status: "active",
      }),
      setDoc(doc(db, "dispatchBatches/batch-a/events/event-a"), {
        type: "batch_created",
      }),
      setDoc(doc(db, "whatsappSessions/session-a"), { step: "village" }),
      setDoc(doc(db, "whatsappProcessedMessages/message-a"), {
...
describe("signed out", () => {
  it("cannot read or write operational data", async () => {
    ...
    await assertFails(
      setDoc(doc(db, "waterRequests/new-request"), { status: "available" }),
    );
  });
});
...
describe("resident", () => {
  ...
  it("updates only the explicit resident profile allowlist", async () => {
    ...
    await assertSucceeds(
      updateDoc(doc(db, "users/resident-a"), {
    ...
    await assertFails(
      updateDoc(doc(db, "users/resident-a"), { roles: ["admin"] }),
    );
    await assertFails(
      updateDoc(doc(db, "users/resident-a"), { authStatus: "unclaimed" }),
    );
    await assertFails(
      updateDoc(doc(db, "users/resident-a"), {
  ...
  it("cannot mutate requests or read raw audit/photo metadata", async () => {
    ...
    const db = dbFor("resident-a");
    await assertFails(
      updateDoc(doc(db, "waterRequests/request-a"), { status: "confirmed" }),
    );
    await assertFails(deleteDoc(doc(db, "waterRequests/request-a")));
    await assertFails(
      addDoc(collection(db, "waterRequests"), { customerId: "resident-a" }),
    );
    await assertFails(
      getDoc(doc(db, "waterRequests/request-a/events/event-a")),
    );
    await assertFails(
      getDoc(doc(db, "waterRequests/request-a/photos/photo-a")),
    );
    await assertFails(
      getDoc(doc(db, "users/resident-a/propertyPhotos/photo-a")),
    );
  });
});
...
describe("driver", () => {
  ...
  it("cannot mutate requests or read raw audit, photo, registry, or offer data", async () => {
    ...
    const db = dbFor("driver-a");
    await assertFails(
      updateDoc(doc(db, "waterRequests/request-a"), { status: "delivered" }),
    );
    await assertFails(
      getDoc(doc(db, "waterRequests/request-a/events/event-a")),
    );
...
describe("viewer", () => {
  it("cannot retrieve raw PII or internal operational records", async () => {
    ...
    await assertFails(getDoc(doc(db, "driverRegistry/registry-a")));
    await assertFails(
      getDoc(doc(db, "driverRegistry/registry-a/events/event-a")),
    );
    await assertFails(
      getDoc(doc(db, "driverRegistry/registry-a/meters/station-a")),
    );
  ...
  it("cannot mutate any operational record", async () => {
    ...
    const db = dbFor("viewer-a");
    await assertFails(
      updateDoc(doc(db, "waterRequests/request-a"), { status: "cancelled" }),
    );
    await assertFails(
      updateDoc(doc(db, "driverRegistry/registry-a"), {
        eligibilityStatus: "eligible",
      }),
    );
    await assertFails(
      updateDoc(doc(db, "config/dispatch"), { offerTimeoutSeconds: 1 }),
    );
  });
});
...
describe.each(["dispatcher-a", "admin-a"])("staff role %s", (uid) => {
  it("has intended reads but cannot bypass server mutation workflows", async () => {
    ...
    await assertSucceeds(
      getDoc(doc(db, "dispatchBatches/batch-a/events/event-a")),
    );
    await assertFails(
      updateDoc(doc(db, "users/resident-a"), { roles: ["admin"] }),
    );
    ...
    await assertFails(
      updateDoc(doc(db, "dispatchBatches/batch-a"), { status: "completed" }),
    );
  });
});
...
describe("locked collections and catch-all", () => {
  it("denies private server-only, uniqueness, photo, and unknown collections", async () => {
    ...
    await assertFails(
      setDoc(doc(db, "unknownCollection/new-doc"), { value: true }),
    );
  });
  ...
  it("allows signed-in users to read fill-station reference data but never write it", async () => {
    ...
    await assertFails(
      updateDoc(doc(db, "fillStations/station-a"), { active: false }),
    );
  });
});
...
describe("Storage deny-all", () => {
  it.each([
    ...
    "dispatcher-a",
    "admin-a",
  ])("denies reads and writes for %s", async (uid) => {
    await seed();
    const context = uid
    ...
    await assertFails(
      storage.ref("property-photos/resident-a/photo-a").getDownloadURL(),
    );
    await assertFails(
      storage
        .ref("property-photos/resident-a/photo-a")
        .putString("photo") as unknown as Promise<unknown>,
    );
    await assertFails(
      storage.ref("request-photos/request-a/photo-a").getDownloadURL(),
    );
    await assertFails(
      storage
        .ref("request-photos/request-a/photo-a")
        .putString("photo") as unknown as Promise<unknown>,
    );
    ...
    await assertFails(
      storage
        .ref("unknown/path")
        .putString("data") as unknown as Promise<unknown>,
    );
  });
});
