...
- Sentry browser fix (issue #119): browser-side error monitoring now actually
  ...
  inline into the browser bundle, so the SDK never started. Production
  ingestion was verified end-to-end (browser + server events, symbolication,
  privacy scrubbing); the temporary admin verification control used for that
  check has been removed. Browser events now also carry the Vercel
  `deploymentId` tag, matching server events.
