...
| Online/offline status | High | unit + emulator | verified | `dispatch.test.ts` assignability; registry availability in `driverRegistryLifecycle.test.ts` | — |
| Assignment selection ordering | Critical | unit + emulator | verified | `dispatch.test.ts` (priority, ties, holds, decline exclusion); `priority.test.ts`; `dispatchBatchSelection.test.ts`; `dispatchAssignment.emulator.test.ts` — canonical ordering across the complete queue via paged streams (>100 candidates, escalation past the old window, catch-all for missing ordering fields, scan bound exhaustion event) | — |
| Atomic assignment (`claimWaterRequest` via `assignNextDeliveryForDriver`) | Critical | emulator + e2e | verified | `dispatchAssignment.emulator.test.ts` two-driver claim race, assignment-before-details, legacy pending-offer expiry; `residentCancellation.emulator.test.ts` claim-vs-cancel races; `driver-workflow.spec.ts` end-to-end | — |
| Release / cooldown / daily limits | High | unit + emulator | verified | `dispatch.test.ts` decline exclusion + `declineResult.test.ts`; `driverRegistryLifecycle.test.ts`; `dispatchAssignment.emulator.test.ts` — release returns work to the queue, declined candidates across page boundaries, all-declined pages do not end the scan | — |
| Assignment-on-visibility invariant (#123) | Critical | emulator | verified | `dispatchAssignment.emulator.test.ts` — details are never returned before the claim commits; concurrent drivers cannot both receive the same request | — |
