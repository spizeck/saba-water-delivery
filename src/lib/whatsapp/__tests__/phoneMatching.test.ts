...
import {
  matchResidentByPhoneFromDirectory,
  normalizePhoneForMatching,
} from "@/lib/whatsapp/phoneMatching";

function resident(
  overrides: Partial<ResidentDirectoryEntry> = {},
...
describe("matchResidentByPhoneFromDirectory", () => {
  it("returns a unique match when exactly one resident's normalized phone matches", () => {
    const directory = [
      ...
      resident({ uid: "uid-2", phone: "+599 000 0000" }),
    ];
  ...
  it("returns none when no resident's phone matches", () => {
    ...
    expect(matchResidentByPhoneFromDirectory("5994165363", directory)).toEqual({
      type: "none",
    });
  });

  it("returns none for an empty directory", () => {
    expect(matchResidentByPhoneFromDirectory("5994165363", [])).toEqual({
      type: "none",
    });
  });
